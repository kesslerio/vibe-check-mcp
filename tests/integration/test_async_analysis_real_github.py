...
class TestAsyncAnalysisRealGitHub:
    ...
    async def test_real_pr_size_detection(self, github_ops):
        ...
        if pr_result.success:
            ...
            logger.info(f"Should use async: {should_async}, Estimated duration: {estimated_duration}s")
        else:
    ...
    async def test_async_workflow_with_mock_massive_pr(self, sample_massive_pr_data):
        ...
        assert result['status'] in ['queued_for_async_analysis', 'not_suitable', 'error']
        ...
        if result['status'] == 'queued_for_async_analysis':
            ...
            if status_result['status'] == 'status_retrieved':
                ...
                assert job_status['status'] in ['queued', 'in_progress', 'completed', 'failed']
            ...
            for i in range(max_checks):
                ...
                if status['status'] == 'status_retrieved':
                    ...
                    logger.info(f"Status check {i+1}: {job_status.get('status')} - {job_status.get('progress_percent', 0)}%")
            ...
            logger.info(f"Async analysis not started: {result.get('error', 'Unknown error')}")
    ...
    async def test_input_validation_integration(self, sample_massive_pr_data):
        ...
        for case in invalid_cases:
            ...
            assert 'validation' in result.get('error', '').lower() or 'invalid' in result.get('error', '').lower()
            logger.info(f"Validation correctly caught: {case['expected_error']}")
    
    @pytest.mark.asyncio
    async def test_concurrent_analysis_requests(self, sample_massive_pr_data, sample_large_pr_data):
...
class TestAsyncAnalysisErrorScenarios:
    ...
    async def test_malformed_pr_data_handling(self):
        ...
        malformed_cases = [
            ...
            {'number': 1, 'title': 'Test'},
            # Negative values
            {'number': 1, 'title': 'Test', 'additions': -100, 'deletions': 50, 'changed_files': 10},
            # Extremely large values
            {'number': 1, 'title': 'Test', 'additions': 999999999, 'deletions': 0, 'changed_files': 1},
            # Wrong data types
            {'number': 1, 'title': 'Test', 'additions': 'many', 'deletions': 50, 'changed_files': 10}
        ]
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        result = await start_async_analysis(999, "kesslerio/vibe-check-mcp", sample_data)
