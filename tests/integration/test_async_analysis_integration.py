...
class TestAsyncAnalysisIntegration:
        ...
        with patch(
            ...
            with patch(
                ...
                result = await start_async_analysis(
                    pr_number=660, repository="owner/repo", pr_data=large_pr_data
                )
                ...
                assert immediate["size_analysis"]["category"] in [
                    "very_large",
                    "massive",
                ]
    ...
    async def test_small_pr_rejection(self):
        ...
        result = await start_async_analysis(
            pr_number=123, repository="owner/repo", pr_data=small_pr_data
        )
    ...
    def test_pr_size_thresholds(self):
        ...
        for additions, deletions, files, expected in test_cases:
            ...
            assert (
                result == expected
            ), f"Failed for {additions}+{deletions} lines, {files} files"
