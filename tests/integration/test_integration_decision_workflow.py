...
class TestIntegrationDecisionWorkflow:
    ...
    def test_cognee_integration_warning_workflow(self):
        ...
        assert (
            "🚨 STOP" in result["recommendation"]
            or "⚠️ CAUTION" in result["recommendation"]
        )
    ...
    def test_decision_framework_workflow(self):
        ...
        official_option = next(
            opt for opt in result["decision_options"] if "Official" in opt["option"]
        )
        custom_option = next(
            opt for opt in result["decision_options"] if "Custom" in opt["option"]
        )
    ...
    def test_safe_integration_workflow(self):
        ...
        assert (
            result["research_required"] == False or result["research_required"] == True
        )  # May still need research
        assert len(result["red_flags_detected"]) == 0
        assert (
            "✅ RESEARCH" in result["recommendation"]
            or "📋 DOCUMENT" in result["recommendation"]
        )
    ...
    def test_unknown_technology_workflow(self):
        ...
        assert (
            "Research required" in result["recommendation"]
            or "📋 DOCUMENT" in result["recommendation"]
        )
...
class TestWorkflowIntegration:
    ...
    def test_alternative_check_to_framework(self):
        ...
        alt_result = check_integration_alternatives(
            technology="cognee", custom_features="REST server, JWT auth"
        )
        ...
        assert (
            framework_result["integration_analysis"]["warning_level"]
...
class TestRealWorldScenarios:
    ...
    def test_cognee_case_study_scenario(self):
        ...
        # Analyze the text
        text_result = analyze_integration_decision_text(
            text, detail_level="comprehensive"
        )
        ...
        assert alt_result["status"] == "success"
        assert (
            "🚨 STOP" in alt_result["recommendation"]
            or "⚠️ CAUTION" in alt_result["recommendation"]
        )
    ...
    def test_mixed_technologies_scenario(self):
        ...
        if claude_recommendations:
            ...
            assert claude_rec["analysis"].warning_level in [
                "caution",
                "warning",
                "critical",
            ]
