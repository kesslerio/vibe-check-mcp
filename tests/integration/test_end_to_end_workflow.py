...
class TestEndToEndWorkflow:
    ...
    def test_complete_vibe_check_workflow_comprehensive_mode(self):
        ...
        with patch('vibe_check.tools.analyze_issue.get_vibe_check_framework') as mock_get_framework:
                    ...
                    "detected_patterns": [
                        {
                            ...
                            "confidence": 0.9,
                            "detected": True,
                            "evidence": ["custom server", "instead of SDK", "API integration"]
                        },
                        {
                            "type": "complexity_escalation",
                            "confidence": 0.7,
                            "detected": True,
                            "evidence": ["sophisticated architecture", "multiple services"]
                        }
                    ],
                    "integration_analysis": {
                        "third_party_services": ["api", "server", "integration"],
                        "infrastructure_keywords": ["custom", "architecture", "implementation"],
                        "complexity_indicators": ["sophisticated", "multiple", "complex"]
                    },
                    "analysis_metadata": {
            ...
            infrastructure_pattern = next(p for p in patterns if p["type"] == "infrastructure_without_implementation")
            assert infrastructure_pattern["confidence"] == 0.9
            assert "custom server" in infrastructure_pattern["evidence"]
            
            complexity_pattern = next(p for p in patterns if p["type"] == "complexity_escalation")
            ...
            assert "infrastructure without implementation" in result["vibe_check"]["claude_reasoning"]
    ...
    def test_workflow_error_handling_and_recovery(self):
        ...
        with patch('vibe_check.tools.analyze_issue.get_vibe_check_framework') as mock_get_framework:
            ...
            mock_get_framework.side_effect = Exception("Framework initialization failed")
    ...
    def test_workflow_with_mixed_confidence_patterns(self):
        ...
        with patch('vibe_check.tools.analyze_issue.get_vibe_check_framework') as mock_get_framework:
            ...
            infra_pattern = next(p for p in patterns if p["type"] == "infrastructure_without_implementation")
            assert infra_pattern["detected"] == True
            assert infra_pattern["confidence"] == 0.6
            
            # Low confidence pattern should not be detected
            doc_pattern = next(p for p in patterns if p["type"] == "documentation_neglect")
