...
class TestMCPTelemetryToolIntegration:
    ...
    async def test_mcp_tool_json_serialization(self):
        ...
        # Add some data
        collector.record_response(RouteType.HYBRID, 200.0, True, "test", 50, 100)
    ...
    def test_mcp_tool_with_large_dataset(self):
        ...
        assert (
            duration < 1.0
        ), f"MCP tool took {duration:.2f} seconds with large dataset"
...
class TestCircuitBreakerIntegration:
    ...
    def test_circuit_breaker_state_changes(self):
        ...
        collector.record_response(
            RouteType.DYNAMIC, 300.0, False, "test2", 50, 0, "TestError"
        )

        # Move to open state
        mock_circuit_breaker.state.value = "open"
        collector.record_response(
            RouteType.DYNAMIC, 0.0, False, "test3", 50, 0, "CircuitBreakerOpenError"
        )
...
class TestVibeCheckMentorIntegration:
    ...
    async def test_mentor_workflow_telemetry(self):
        ...
        cache_metric = next(
            m for m in collector._recent_metrics if m.route_type == RouteType.CACHE_HIT
        )
        dynamic_metric = next(
            m for m in collector._recent_metrics if m.route_type == RouteType.DYNAMIC
        )
    ...
    def test_manual_telemetry_context_in_workflow(self):
        ...
        def complex_mentor_workflow(query: str) -> dict:
            ...
            with TelemetryContext(
                RouteType.STATIC, "intent_detection", len(query)
            ) as ctx:
                time.sleep(0.005)
                intent = "architecture_decision"
            ...
            with TelemetryContext(
                RouteType.HYBRID, "route_selection", len(query)
            ) as ctx:
                time.sleep(0.003)
            ...
            with TelemetryContext(
                RouteType.DYNAMIC, "response_generation", len(query)
            ) as ctx:
                time.sleep(0.01)
...
class TestTelemetryResilience:
    ...
    def test_telemetry_continues_on_component_errors(self):
        ...
        try:
            ...
            pytest.skip(
                "Circuit breaker error handling needs improvement in telemetry system"
            )
