...
def register_claude_cli_test_tool(mcp: FastMCP) -> None:
    ...
    async def test_claude_cli_integration(
        ...
        if debug_mode:
            logger.info(
                f"Starting external Claude CLI test with prompt: {test_prompt[:50]}..."
            )
        ...
        try:
            ...
            result: ClaudeCliResult = await external_cli.analyze_content(
                content=test_prompt, task_type=task_type
            )
            ...
            if debug_mode:
                logger.info(
                    f"External CLI execution completed in {execution_time:.2f}s"
                )
            ...
            if result.success and result.output:
                ...
                validation = {
                    ...
                    "appears_reasonable": len(output_text) > 0
                    and len(output_text) < 50000,
                    "contains_expected_answer": (
                        "4" in output_text if "2+2" in test_prompt else None
                    ),
            ...
            return ClaudeCliTestResponse(
                ...
                or f"ExternalClaudeCli.analyze_content(task_type='{task_type}')",
...
"""
        ...
        try:
            ...
            command = [
                "claude",
                "-p",
                f"Analyze this Python code for any issues: {test_file}",
            ]
    ...
    async def test_claude_cli_comprehensive() -> Dict[str, Any]:
        ...
        for scenario in test_scenarios:
            try:
                ...
                scenario_result = {
                    ...
                    "output_preview": (
                        test_result.output[:100] if test_result.output else None
                    ),
        ...
        results["summary"] = (
            f"Passed {results['tests_passed']}/{total_tests} tests. "
            + (
    ...
    async def test_claude_cli_mcp_permissions() -> Dict[str, Any]:
        ...
        command = ["claude", "-p", test_prompt, "--dangerously-skip-permissions"]
        ...
        try:
            ...
            return {
                ...
                "validation": {
                    ...
                    "permissions_bypassed": "--dangerously-skip-permissions"
                    in " ".join(command),
                    "output_reasonable": (
                        result.stdout and len(result.stdout.strip()) > 10
                        if result.stdout
                        else False
                    ),
                },
