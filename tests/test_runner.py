...
class TestRunner:
    ...
    def _run_tests_sequential(self, verbose: bool) -> Dict[str, Any]:
        ...
        for test_method in test_methods:
            print(f"\n{'='*60}")
            print(f"Running {test_method.__name__.replace('run_', '').replace('_', ' ').title()}")
            print('='*60)
            ...
            if result['success']:
                print(f"✅ {result['category']} tests passed ({result['duration']:.2f}s)")
            else:
                print(f"❌ {result['category']} tests failed ({result['duration']:.2f}s)")
    ...
    def run_quality_checks(self) -> Dict[str, Any]:
        ...
        mypy_result = subprocess.run([
            sys.executable, "-m", "mypy", "src/vibe_check", "--ignore-missing-imports"
        ...
        try:
            flake8_result = subprocess.run([
                sys.executable, "-m", "flake8", "src/vibe_check", "--max-line-length=100"
            ], capture_output=True, text=True, cwd=self.project_root)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run comprehensive vibe-check-mcp tests")
    parser.add_argument("--category", choices=[
        "unit", "integration", "security", "performance", 
        "edge_case", "novel_query", "e2e", "all"
