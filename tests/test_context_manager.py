...
class TestSecurityValidator:
    ...
    def test_validate_relative_path(self, tmp_path):
        ...
        is_valid, resolved_path, error = SecurityValidator.validate_path(
            "test.py", working_directory=str(tmp_path)
        )
        assert is_valid
    ...
    def test_prevent_path_traversal(self, tmp_path):
        ...
        is_valid, resolved_path, error = SecurityValidator.validate_path(
            "../outside.py", working_directory=str(tmp_path)
        )
        assert not is_valid
        assert "outside working directory" in error
...
"""
        ...
        assert any(
            "TODO" in line[1] or "FIXME" in line[1]
            for line in relevant["potential_issues"]
        )
...
class TestContextCache:
    ...
    def test_file_caching(self, tmp_path):
        ...
        contexts1, _ = cache.add_files_to_session(
            "test-session", [str(test_file)], working_directory=str(tmp_path)
        )
    ...
    def test_max_files_limit(self, tmp_path):
        ...
        contexts, errors = cache.add_files_to_session(
            "test-session", files, working_directory=str(tmp_path)
        )
    ...
    def test_cache_stats(self, tmp_path):
        ...
        cache.add_files_to_session(
            "session1", [str(test_file)], working_directory=str(tmp_path)
        )
        cache.add_files_to_session(
            "session2", [str(test_file)], working_directory=str(tmp_path)
        )
...
"""
        ...
        assert (
            len(file_context.relevant_lines["potential_issues"]) > 0
        )  # Should find TODO
