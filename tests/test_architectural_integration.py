...
class TestArchitecturalIntegration:
    ...
    def test_real_github_issue_examples(self):
        ...
        for example in real_issue_examples:
            ...
            result = detector.detect_concepts(example['body'], f"Title: {example['title']}")
            ...
            assert len(result.github_search_queries) > 0, f"No search guidance for: {example['title']}"
    ...
    def test_end_to_end_issue_analysis_integration(self):
        ...
        with patch.object(analyzer, '_fetch_issue_data', return_value=mock_issue_data):
            ...
            auth_guidance_found = any("authentication" in action.lower() for action in actions)
            assert auth_guidance_found, "Expected authentication-specific guidance in recommended actions"
    ...
    def test_architectural_recommendations_quality(self):
        ...
        for concept_name, test_text in concept_tests:
            ...
            if concept_name == "authentication":
                assert any(keyword in rec_text for keyword in ["jwt", "token", "session", "password"])
            elif concept_name == "payment":
                assert any(keyword in rec_text for keyword in ["stripe", "webhook", "transaction", "billing"])
            elif concept_name == "api":
                assert any(keyword in rec_text for keyword in ["endpoint", "route", "middleware", "validation"])
            elif concept_name == "database":
                assert any(keyword in rec_text for keyword in ["query", "connection", "index", "migration"])
            elif concept_name == "caching":
                assert any(keyword in rec_text for keyword in ["redis", "ttl", "invalidation", "cache"])

    def test_github_search_query_generation_quality(self):
        """Test that generated GitHub search queries are well-formed and useful"""
        detector = ArchitecturalConceptDetector()
        
        result = detector.detect_concepts("Authentication middleware is causing timeouts")
        
        assert len(result.github_search_queries) > 0
        
        # Verify queries contain relevant search terms
        queries_text = ' '.join(result.github_search_queries).lower()
        assert any(term in queries_text for term in ["auth", "login", "jwt", "token", "session"])
    ...
    def test_confidence_scoring_accuracy(self):
        ...
        high_confidence_text = "JWT authentication token validation in login session middleware"
