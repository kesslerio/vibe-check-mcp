...
class TestSecurityValidator:
    ...
    def test_path_traversal_prevention(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            is_valid, path, error = SecurityValidator.validate_path(
                "../../../etc/passwd", working_directory=tmpdir
            )
    ...
    def test_symlink_validation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            is_valid, path, error = SecurityValidator.validate_path(
                str(symlink), working_directory=tmpdir
            )
    ...
    def test_file_size_limit(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            is_valid, path, error = SecurityValidator.validate_path(
                str(large_file), working_directory=tmpdir
            )
    ...
    def test_allowed_extensions(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            is_valid, path, error = SecurityValidator.validate_path(
                str(allowed_file), working_directory=tmpdir
            )
            assert is_valid
            ...
            is_valid, path, error = SecurityValidator.validate_path(
                str(disallowed_file), working_directory=tmpdir
            )
            assert not is_valid
    ...
    def test_directory_validation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            is_valid, path, error = SecurityValidator.validate_path(
                str(subdir), working_directory=tmpdir
            )
...
"""
        ...
        relevant = CodeParser.extract_relevant_context(
            content, "process user validation", "python"
        )
...
class TestContextCache:
    ...
    def test_total_size_limit(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            contexts, errors = cache.add_files_to_session(
                "test-session", large_files, working_directory=tmpdir
            )

            # Should have errors about too many files
            assert len(errors) > 0
            assert (
                "Too many files" in str(errors[0]) or len(contexts) == 10
            )  # MAX_FILES
    ...
    def test_session_expiry(self):
        ...
        with patch(
            "time.time", return_value=session.created_at + 7200
        ):  # 2 hours later
...
"""
            ...
            with patch.dict(os.environ, {"WORKSPACE": tmpdir}):
                from vibe_check.tools.vibe_mentor_workspace import (
                    WorkspaceAwareMentorEngine,
                )
