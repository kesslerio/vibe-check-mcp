...
class TestSecurityRegressionPR190:
    ...
    def test_template_injection_prevention(self):
        ...
        injection_attempts = [
            ...
            {"name": "{% for x in range(1000000) %}x{% endfor %}", "task": "normal"},  # DoS
            {"name": "__import__('os').system('whoami')", "task": "normal"},  # Python injection
        ]
        ...
        with tempfile.TemporaryDirectory() as workspace:
            ...
            with patch.dict(os.environ, {'WORKSPACE': workspace}):
                ...
                for sensitive_path in sensitive_paths:
                    ...
                    assert not is_valid
                    assert "outside working directory" in error or "not allowed" in error
    ...
    def test_security_patches_applied_on_import(self):
        ...
        assert hasattr(mcp_sampling.MCPSamplingClient, 'rate_limiter') or \
               'rate_limiter' in str(mcp_sampling.MCPSamplingClient.__init__)
        ...
        assert "{{evil}}" not in result or result == "Test {{evil}}"  # Escaped or literal
    ...
    async def test_security_monitoring_and_logging(self):
        ...
        security_logs = [
            ...
            if 'redacted' in str(log.message).lower() or 
               'security' in str(log.message).lower() or
               'injection' in str(log.message).lower()
        ]
...
class TestSecurityWithFixtures:
    ...
    def test_symlink_attack_prevention(self, mock_secure_environment):
        ...
        is_valid, path, error = SecurityValidator.validate_path(
            str(symlink),
            working_directory=str(env["workspace"])
        )
