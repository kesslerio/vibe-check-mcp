    ...
    def _check_http_endpoint(self, port: int) -> bool:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=2) as response:
                return (
                    response.status == 200 or response.status == 406
                )  # 406 is expected for SSE endpoint
...
class TestMCPCompatibility:
    ...
    def test_claude_desktop_config_compatibility(self):
        ...
        assert (
            "Starting Vibe Check MCP Server" in result.stderr
            or "Starting Vibe Check MCP Server" in result.stdout
        )
