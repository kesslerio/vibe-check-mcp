...
Fixes #123""",
            ...
            "files": [
                ...
                {
                    "path": "tests/test_auth_endpoints.py",
                    "additions": 55,
                    "deletions": 5,
                },
    ...
    def test_medium_pr_content_format_unchanged(
    ...
    @pytest.mark.asyncio
    async def test_small_pr_analysis_workflow_unchanged(
        self, pr_tool, classic_small_pr_data, review_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_medium_pr_analysis_workflow_unchanged(
        self, pr_tool, classic_medium_pr_data, review_context
    ):
        ...
        assert (
            combined_size < 50000
        ), f"Medium PR test data should be under threshold, got {combined_size}"
    ...
    def test_timeout_calculation_backward_compatibility(self, pr_tool):
        ...
        assert (
            60 <= small_timeout <= 300
        ), f"Small PR timeout should be 60-300s, got {small_timeout}"
        assert (
            60 <= medium_timeout <= 300
        ), f"Medium PR timeout should be 60-300s, got {medium_timeout}"
    ...
    def test_performance_not_degraded(
        self, pr_tool, classic_small_pr_data, classic_medium_pr_data, review_context
    ):
        ...
        assert (
            small_time < 0.1
        ), f"Small PR content creation took {small_time:.3f}s, should be <0.1s"
        assert (
            medium_time < 0.2
        ), f"Medium PR content creation took {medium_time:.3f}s, should be <0.2s"
    ...
    def test_linked_issue_handling_preserved(
        self, pr_tool, classic_medium_pr_data, review_context
    ):
