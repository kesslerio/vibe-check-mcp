...
class TestInputValidation:
    ...
    def test_repository_validation_basic(self):
        ...
        assert AsyncAnalysisValidator.validate_repository("user-name/repo-name").is_valid
        assert AsyncAnalysisValidator.validate_repository("org.name/project.name").is_valid
    ...
    def test_repository_validation_consecutive_dots(self):
        ...
        assert not AsyncAnalysisValidator.validate_repository("owner..name/repo").is_valid
        assert not AsyncAnalysisValidator.validate_repository("owner/repo..name").is_valid
        assert not AsyncAnalysisValidator.validate_repository("owner../repo").is_valid
        
        # Should accept single dots
        assert AsyncAnalysisValidator.validate_repository("owner.name/repo.name").is_valid
    
    def test_repository_validation_length_limits(self):
        """Test repository name length limits."""
        # Test maximum length
        long_name = "a" * 150  # Exceeds limit
        assert not AsyncAnalysisValidator.validate_repository(f"owner/{long_name}").is_valid
        
        # Test reasonable length
        normal_name = "a" * 50
        assert AsyncAnalysisValidator.validate_repository(f"owner/{normal_name}").is_valid
    ...
    def test_pr_number_validation(self):
        ...
        assert AsyncAnalysisValidator.validate_pr_number("123").is_valid  # String conversion
        ...
        assert not AsyncAnalysisValidator.validate_pr_number(10000000).is_valid  # Too large
    ...
    def test_job_id_validation(self):
        ...
        assert not AsyncAnalysisValidator.validate_job_id("a" * 150).is_valid  # Too long
    ...
    def test_suspicious_pattern_detection(self):
        ...
        assert not AsyncAnalysisValidator.validate_repository("owner/repo; DROP TABLE").is_valid
        assert not AsyncAnalysisValidator.validate_job_id("job' UNION SELECT").is_valid
        
        # Script injection patterns
        assert not AsyncAnalysisValidator.validate_repository("owner/<script>alert</script>").is_valid
        
        # Path traversal patterns
        assert not AsyncAnalysisValidator.validate_job_id("../../../etc/passwd").is_valid
    ...
    def test_complete_request_validation(self):
        ...
        # Valid request
        is_valid, result = validate_async_analysis_request(123, "owner/repo", valid_pr_data)
        assert is_valid
        ...
        # Invalid PR number
        is_valid, result = validate_async_analysis_request(-1, "owner/repo", valid_pr_data)
        assert not is_valid
        assert result["field"] == "pr_number"
        
        # Invalid repository
        is_valid, result = validate_async_analysis_request(123, "invalid..repo", valid_pr_data)
        assert not is_valid
...
class TestResourceMonitoring:
    ...
    def test_new_job_acceptance_logic(self):
        ...
        if not can_accept:
            assert len(reason) > 0
            assert any(keyword in reason.lower() for keyword in ["limit", "exceeded", "capacity", "violation"])
        else:
...
class TestHealthMonitoring:
    ...
    async def test_health_monitor_comprehensive_check(self):
        ...
        with patch.object(monitor, '_check_system_initialization', side_effect=lambda: mock_check("system")):
            with patch.object(monitor, '_check_queue_health', side_effect=lambda: mock_check("queue")):
                with patch.object(monitor, '_check_worker_health', side_effect=lambda: mock_check("worker")):
                    with patch.object(monitor, '_check_resource_monitoring', side_effect=lambda: mock_check("resource")):
                        with patch.object(monitor, '_check_github_api', side_effect=lambda: mock_check("github")):
...
class TestGracefulDegradation:
    ...
    def test_fallback_config_creation(self):
        ...
        config = FallbackConfig(
            ...
            enable_fast_analysis_fallback=True
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if test_category == "validation":
            ...
            print("Available categories: validation, resources, health, degradation, integration")
    else:
        # Run all tests
