...
class TestDoomLoopDetector:
    ...
    def test_mcp_call_tracking(self):
        ...
        detector.track_mcp_call(
            "analyze_issue",
            "Need to choose between React and Vue",
            "frontend framework",
        )
        detector.track_mcp_call(
            "analyze_issue", "Should we use Redux or Context", "state management"
        )
        detector.track_mcp_call(
            "analyze_issue", "What about TypeScript vs JavaScript", "typing"
        )
    ...
    def test_repetition_pattern_detection(self):
        ...
        for i in range(6):
            detector.track_mcp_call(
                "analyze_issue", f"iteration {i}", "repeated analysis"
            )
    ...
    def test_decision_paralysis_detection(self):
        ...
        for phrase in decision_phrases:
            detector.track_mcp_call(
                "analyze_issue", f"Content with {phrase}", "decision making"
            )
    ...
    def test_warning_level_calculation(self):
        ...
        for i in range(8):
            detector.track_mcp_call(
                "analyze_issue", f"Should we decide iteration {i}", "decisions"
            )
    ...
    def test_intervention_recommendations(self):
        ...
        for i in range(6):
            detector.track_mcp_call(
                "integration_analysis", f"Custom vs official {i}", "integration"
            )
...
class TestPerformanceAndReliability:
    ...
    def test_large_content_analysis_performance(self):
        ...
        large_content = (
            "We need to decide between options. " * 1000
    ...
    def test_concurrent_session_tracking(self):
        ...
        def analyze_content(content_id):
            ...
            result = analyze_text_for_doom_loops(
                content, tool_name=f"test_{content_id}"
            )
