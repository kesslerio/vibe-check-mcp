...
class TestCodeReferenceExtractor:
    ...
    def test_extract_stack_trace_nodejs(self):
        ...
        stack_ref = next(
            (r for r in stack_refs if r.value == "/app/orders/processor.js"), None
        )
    ...
    def test_path_validation_security(self):
        ...
        assert (
            len(deep_paths) == 0
        ), f"Found deep paths that should be filtered: {deep_paths}"
    ...
    def test_malformed_stack_traces(self):
        ...
        nodejs_trace = next(
            (r for r in stack_refs if "/app/orders/valid.js" in r.value), None
        )
    ...
    def test_very_long_file_paths(self):
        ...
        config = ExtractionConfig(
            max_file_path_length=100, enable_metrics_logging=False
        )
        extractor = CodeReferenceExtractor(config)
