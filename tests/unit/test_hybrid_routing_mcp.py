...
def router_with_speed_preference():
    ...
    return HybridRouter(
        confidence_threshold=0.7, enable_caching=True, prefer_speed=True
    )
...
def router_no_cache():
    ...
    return HybridRouter(
        confidence_threshold=0.7, enable_caching=False, prefer_speed=False
    )
...
def circuit_breaker():
    ...
    return CircuitBreaker(failure_threshold=3, recovery_timeout=30, success_threshold=2)
...
class TestCommonQueryRouting:
    ...
    def test_react_vs_vue_routes_static(self, router, common_queries):
        ...
        assert metrics.confidence >= 0.7
        assert (
            "Common pattern" in metrics.reasoning
            or "Common intent" in metrics.reasoning
        )
...
class TestBorderlineConfidence:
    ...
    def test_borderline_with_speed_preference_routes_hybrid(
        self, router_with_speed_preference, borderline_queries
    ):
    ...
    def test_borderline_without_speed_preference_routes_dynamic(
        self, router, borderline_queries
    ):
    ...
    def test_confidence_threshold_adjustment(self, router):
        ...
        feedback = [{"decision": "static", "success": False} for _ in range(15)]
...
class TestMCPSamplingFallback:
    ...
    def test_fallback_on_generation_failure(self, router):
        ...
        should_fallback = router.should_fallback(
            metrics=metrics, generation_failed=True, latency_ms=1000
        )
    ...
    def test_fallback_on_high_latency(self, router):
        ...
        should_fallback = router.should_fallback(
            metrics=metrics, generation_failed=False, latency_ms=6000  # >5s threshold
        )
    ...
    def test_no_fallback_when_unavailable(self, router):
        ...
        should_fallback = router.should_fallback(
            metrics=metrics, generation_failed=True, latency_ms=6000
        )
...
class TestLatencyRequirements:
    ...
    def test_timeout_enforcement(self, mcp_client):
        ...
        config = SamplingConfig(
            temperature=0.7, max_tokens=500, quality=ResponseQuality.FAST
        )
...
class TestCacheScenarios:
    ...
    def test_cache_miss_on_different_query(self, router):
        ...
        # First query
        router.decide_route(
            "Query 1", "intent1", {"technologies": ["tech1"]}, False, True
        )

        # Different query - should miss cache
        router.decide_route(
            "Query 2", "intent2", {"technologies": ["tech2"]}, False, True
        )
    ...
    def test_response_cache_operations(self, response_cache):
        ...
        cached = response_cache.get(
            intent="test_intent", query="test query", context={"tech": ["python"]}
        )
        ...
        cached = response_cache.get(
            intent="different_intent", query="test query", context={"tech": ["python"]}
        )
...
class TestWorkspaceContextInfluence:
    ...
    def test_workspace_forces_dynamic_for_specific_queries(
        self, router, workspace_context_data
    ):
        ...
        assert metrics.confidence < 0.7
        assert (
            "specific files" in metrics.reasoning.lower()
            or "workspace" in metrics.reasoning.lower()
        )
    ...
    def test_file_reference_detection(self, router):
        ...
        assert metrics.confidence < 0.7
        assert (
            "specific files" in metrics.reasoning.lower() or ".py" in metrics.reasoning
        )
...
class TestIntegrationScenarios:
    ...
    async def test_novel_query_full_flow(self, router, mcp_client):
        ...
        query = (
            "Integrate our custom ESLint rules with TypeScript 5.0 and new decorators"
        )
    ...
    async def test_fallback_flow(self, router, mcp_client):
        ...
        with patch.object(mcp_client, "generate_dynamic_response") as mock_generate:
            ...
            response = await mcp_client.generate_dynamic_response(
                intent="complex", query=query, context={}, workspace_data=None, ctx=None
            )
        ...
        should_fallback = router.should_fallback(
            metrics=metrics, generation_failed=True, latency_ms=0
        )
...
if __name__ == "__main__":
    pytest.main(
        [
            __file__,
            "-v",
            "--cov=vibe_check.mentor.hybrid_router",
            "--cov=vibe_check.mentor.mcp_sampling",
        ]
    )
