...
class TestAsyncAnalysisWorker:
    ...
    async def test_worker_process_job_success(self):
        ...
        # Create a test job
        pr_data = {"title": "Test", "additions": 1000, "deletions": 500, "changed_files": 20}
        job = AnalysisJob("job-1", 1, "owner/repo", pr_data)
        
        # Mock GitHub operations and analysis
        with patch('src.vibe_check.tools.async_analysis.worker.get_default_github_operations') as mock_github:
            mock_github_ops = Mock()
            mock_github_ops.get_pull_request_files.return_value = Mock(
                success=True,
                data=[{"filename": "test.py", "patch": "test content"}]
            )
            mock_github.return_value = mock_github_ops
            
            # Mock chunked analysis
            with patch('src.vibe_check.tools.async_analysis.worker.ChunkedAnalysisCoordinator') as mock_coordinator:
                mock_result = Mock()
                ...
                mock_coordinator.return_value.analyze_pr_chunked.return_value = mock_result
