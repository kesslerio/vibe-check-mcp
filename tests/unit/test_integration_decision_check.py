...
class TestIntegrationKnowledgeBase:
    ...
    def test_load_knowledge_base_success(
        self, mock_exists, mock_file, sample_knowledge_base
    ):
    ...
    def test_get_technology_info(self, sample_knowledge_base):
        ...
        with patch.object(
            ...
            return_value=sample_knowledge_base,
        ):
            kb = IntegrationKnowledgeBase()
    ...
    def test_detect_red_flags(self, sample_knowledge_base):
        ...
        with patch.object(
            ...
            assert (
                "Custom custom REST server (official alternative available)"
                in red_flags
            )
            assert (
                "Custom manual JWT implementation (official alternative available)"
                in red_flags
            )
...
class TestValidationQuestions:
    ...
    def test_generate_validation_questions_api(self):
        ...
        questions = generate_validation_questions(
            "service", ["api endpoints", "REST API"]
        )
...
class TestRecommendationGeneration:
    ...
    def test_stop_recommendation_high_red_flags(self):
        ...
        technology_info = {
            ...
            "official_container": "tech/container",
        }
        custom_features = ["flag1 custom", "flag2 custom"]

        recommendation = generate_recommendation(
            "tech", custom_features, technology_info
        )
...
class TestOfficialAlternativesCheck:
    ...
    def test_check_cognee_integration(self, mock_load, sample_knowledge_base):
        ...
        assert (
            "🚨 STOP" in result.recommendation or "⚠️ CAUTION" in result.recommendation
        )
