...
class TestVibeCheckFrameworkClaudeIntegration:
    ...
    def test_claude_analysis_execution_success(
        self, mock_temp_file, mock_subprocess, framework
    ):
    ...
    @patch("subprocess.run")
    @patch("tempfile.NamedTemporaryFile")
    def test_claude_analysis_execution_failure(
        self, mock_temp_file, mock_subprocess, framework
    ):
    ...
    def test_claude_prompt_generation_comprehensive(self, framework):
        ...
        prompt = framework._generate_claude_prompt(
            issue_data, DetailLevel.COMPREHENSIVE
        )
    ...
    def test_claude_security_prompt_sanitization(self, mock_temp_file, framework):
        ...
        prompt = framework._generate_claude_prompt(
            dangerous_issue, DetailLevel.STANDARD
        )
