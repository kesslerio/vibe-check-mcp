...
class TestPromptBuilder:
    ...
    def test_build_prompt_architecture_decision(self):
        ...
        assert (
            "architecture_decision" in prompt.lower()
            or "architecture" in prompt.lower()
        )
    ...
    def test_map_intent_to_template(self):
        ...
        assert (
            PromptBuilder._map_intent_to_template("architecture")
            == "architecture_decision"
        )
        assert (
            PromptBuilder._map_intent_to_template("design pattern")
            == "architecture_decision"
        )
        ...
        assert (
            PromptBuilder._map_intent_to_template("implement feature")
            == "implementation_guide"
        )
        assert (
            PromptBuilder._map_intent_to_template("build api") == "implementation_guide"
        )
...
class TestResponseCache:
    ...
    def test_cache_miss(self):
        ...
        result = cache.get(intent="nonexistent", query="not in cache", context={})
...
class TestHybridRouter:
    ...
    def test_should_fallback(self):
        ...
        assert not router.should_fallback(
            metrics, generation_failed=False, latency_ms=2000
        )
...
class TestRouteOptimizer:
    ...
    def test_pattern_recommendation(self):
        ...
        recommendation = optimizer.get_pattern_recommendation(
