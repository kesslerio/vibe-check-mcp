...
class TestAdaptivePromptSizing:
    ...
    def test_threshold_detection_small_pr(self, pr_tool, small_pr_data, review_context):
        ...
        assert (
            combined_size < 50000
        ), f"Small PR should be under threshold, got {combined_size} chars"
    ...
    def test_diff_pattern_extraction_limits(self, pr_tool):
        ...
        assert (
            len(pattern_lines) <= 200
        ), f"Expected ≤200 lines, got {len(pattern_lines)}"
    ...
    @pytest.mark.asyncio
    async def test_adaptive_sizing_integration_small_pr(
    ...
    @pytest.mark.asyncio
    async def test_adaptive_sizing_integration_large_pr(
        self, pr_tool, large_pr_data, review_context
    ):
    ...
    def test_content_reduction_effectiveness(
        self, pr_tool, large_pr_data, review_context
    ):
        ...
        assert (
            len(summary_content) > 1000
        ), "Summary should still contain substantial content"
    ...
    def test_adaptive_timeout_calculation(self, pr_tool):
        ...
        for timeout in [small_timeout, medium_timeout, large_timeout]:
            assert (
                60 <= timeout <= 600
            ), f"Timeout {timeout} should be between 60-600 seconds"

    def test_summary_preserves_critical_data(
        self, pr_tool, large_pr_data, review_context
    ):
