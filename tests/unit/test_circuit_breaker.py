...
class TestCircuitBreaker:
    ...
    def test_circuit_breaker_initialization(self):
        ...
        cb = ClaudeCliCircuitBreaker(
            failure_threshold=5, recovery_timeout=300, name="test_breaker"
        )
...
class TestRetryLogic:
    ...
    async def test_retry_with_circuit_breaker_open(self):
        ...
        cb = ClaudeCliCircuitBreaker(
            failure_threshold=2
        )  # Allow one retry before opening
...
class TestHealthMonitor:
    ...
    def test_health_status_degraded(self):
        ...
        assert status.level in [
            "DEGRADED",
            "UNHEALTHY",
        ]  # Allow both as multiplicative factors can vary
...
class TestIntegration:
    ...
    async def test_end_to_end_integration(self):
        ...
        async def flaky_service():
            ...
            if call_count <= 2:
                monitor.record_call(
                    success=False, duration=1.0, error_type="ServiceError"
                )
...
async def test_circuit_breaker_with_timeout():
    ...
    config = RetryConfig(
        max_retries=1, base_delay=0.05
    )  # Shorter delay for faster test
    executor = RetryExecutor(cb, config)

    # Should timeout and be recorded as failure
    with pytest.raises(ClaudeCliError):
        await executor.execute_with_retry(
            slow_function, timeout=0.05
