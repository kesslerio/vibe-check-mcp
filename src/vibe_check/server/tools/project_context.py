...
def detect_project_libraries(
    ...
    try:
        ...
        context_manager.detection_engine.config.context_loading.library_detection.max_files_to_scan = (
            max_files
        )
        context_manager.detection_engine.config.context_loading.library_detection.timeout_seconds = (
            timeout_seconds
        )

        # Perform detection
        detection_result = context_manager.detection_engine.scan_project_files(
            project_root
        )
...
def load_project_context(
    ...
    try:
        ...
        return {
            ...
            "context": {
                "libraries": (
                    list(context.library_docs.keys()) if include_libraries else []
                ),
                "project_conventions": (
                    context.project_conventions if include_docs else {}
                ),
...
def create_vibe_check_directory_structure(
    ...
    try:
        ...
        if vibe_check_dir.exists():
            created_files = [
                str(f.relative_to(vibe_check_dir))
                for f in vibe_check_dir.rglob("*")
                if f.is_file()
            ]
