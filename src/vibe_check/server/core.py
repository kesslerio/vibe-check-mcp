...
try:
    ...
    print(
        "Using official MCP server FastMCP implementation for Claude Code compatibility"
    )
except ImportError:
    try:
...
def get_mcp_instance() -> FastMCP:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                mcp = FastMCP(
                    name="Vibe Check MCP", version=version  # Use dynamic version
                )
        ...
        if original_log_level:
            ...
            if original_log_level.lower() in [
                "debug",
                "info",
                "warning",
                "error",
