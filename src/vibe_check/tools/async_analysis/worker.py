...
class AsyncAnalysisWorker:
    ...
    async def start(self):
        ...
        while self.running:
            try:
                ...
                if self.current_job:
                    self.queue.fail_job(
                        self.current_job.job_id, f"Worker error: {str(e)}"
                    )
    ...
    async def _process_job(self, job: AnalysisJob) -> WorkerResult:
        ...
        logger.info(
            ...
            extra={
                ...
                "pr_size": job.pr_data.get("additions", 0)
                + job.pr_data.get("deletions", 0),
        ...
        try:
            ...
            self.queue.update_job_progress(
                job.job_id, self.config.progress_checkpoints["started"], self.worker_id
            )
            ...
            files_result = github_ops.get_pull_request_files(
                job.repository, job.pr_number
            )
            ...
            return WorkerResult(
                success=True, job_id=job.job_id, duration=duration, result=final_result
            )
            ...
            return WorkerResult(
                success=False, job_id=job.job_id, duration=duration, error=error_message
            )

    async def _analyze_large_pr(
        self, job: AnalysisJob, pr_files: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        chunked_result = await self.chunked_analyzer.analyze_pr_chunked(
            pr_data=job.pr_data, pr_files=pr_files
        )
    ...
    def _prepare_final_result(
        self, job: AnalysisJob, analysis_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "patterns_detected": (
                chunked_result.patterns_detected if chunked_result else []
            ),
            ...
            "chunk_analysis": {
                ...
                "successful_chunks": (
                    chunked_result.successful_chunks if chunked_result else 0
                ),
                "failed_chunks": chunked_result.failed_chunks if chunked_result else 0,
                "total_duration": (
                    chunked_result.total_duration if chunked_result else 0
                ),
                "chunk_summaries": (
                    chunked_result.chunk_summaries if chunked_result else []
                ),
            },
            # PR metadata
            "pr_info": {
                ...
                "total_changes": job.pr_data.get("additions", 0)
                + job.pr_data.get("deletions", 0),
            },
            # Processing metadata
            "processing_info": {
...
class AsyncWorkerManager:
    ...
    def __init__(self, queue: AsyncAnalysisQueue, config: AsyncAnalysisConfig = None):
        self.queue = queue
        self.config = config or DEFAULT_ASYNC_CONFIG
    ...
    async def start_workers(self):
        ...
        for i in range(self.config.max_concurrent_workers):
            worker = AsyncAnalysisWorker(
                queue=self.queue, worker_id=f"worker-{i+1}", config=self.config
            )
        ...
        return [
            {
                ...
                "current_pr": (
                    f"{worker.current_job.repository}#{worker.current_job.pr_number}"
