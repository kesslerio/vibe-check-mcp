...
class SystemMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_metrics": {
                "average_job_duration_seconds": round(self.average_job_duration_seconds, 2),
...
class HealthMonitor:
    ...
    async def collect_system_metrics(self) -> SystemMetrics:
        ...
        try:
            ...
            metrics.success_rate_percent = metrics_data.get("success_rate_percent", 100.0)
            metrics.average_job_duration_seconds = metrics_data.get("average_duration_seconds", 0.0)
            
            # Calculate throughput (jobs per hour)
            if metrics.completed_jobs > 0 and metrics.average_job_duration_seconds > 0:
                metrics.throughput_jobs_per_hour = 3600 / metrics.average_job_duration_seconds
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        critical_count = sum(1 for r in latest_results.values() if r.status == "critical")
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        if len(self.metrics_history) >= 10:
            ...
            oldest_10 = self.metrics_history[-20:-10] if len(self.metrics_history) >= 20 else []
            
            if oldest_10:
                # Calculate average job duration trend
                recent_avg_duration = sum(m.average_job_duration_seconds for m in recent_10) / len(recent_10)
                old_avg_duration = sum(m.average_job_duration_seconds for m in oldest_10) / len(oldest_10)
                
                if old_avg_duration > 0:
                    duration_trend = ((recent_avg_duration - old_avg_duration) / old_avg_duration) * 100
                    trends["duration_change_percent"] = round(duration_trend, 2)
                
                # Calculate success rate trend
                recent_success = sum(m.success_rate_percent for m in recent_10) / len(recent_10)
                old_success = sum(m.success_rate_percent for m in oldest_10) / len(oldest_10)
    ...
    async def _check_queue_health(self) -> HealthCheckResult:
        ...
        try:
            ...
            if utilization > 90:
                status = "critical"
                message = f"Queue nearly full: {queue_size}/{max_size} ({utilization:.1f}%)"
            elif utilization > 70:
                status = "warning"
    ...
    async def _check_resource_monitoring(self) -> HealthCheckResult:
        ...
        try:
            ...
            if not status_info["monitoring_active"]:
                ...
                status = "warning"
                message = f"Resource violations detected: {status_info['total_violations']}"
            else:
                status = "healthy"
        ...
        try:
            ...
            return HealthCheckResult(
                ...
                details={
                    ...
                    "implementation": auth_result.data.get("implementation") if auth_result.data else None
                },
                response_time_ms=response_time
            )
