        ...
        for feature in custom_features:
            ...
            for flag in red_flags:
                if flag.lower() in feature_lower:
                    detected_flags.append(f"Custom {feature} (official alternative available)")
        
        return detected_flags
    
    def analyze_feature_coverage(self, custom_features: List[str], technology_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for custom_feature in custom_features:
            ...
            for official_feature in official_features:
                if (custom_lower in official_feature.lower() or 
                    official_feature.lower() in custom_lower):
                    coverage_analysis["covered_by_official"].append({
                        "custom": custom_feature,
                        "official": official_feature
                    })
                    covered = True
                    break
        ...
        if custom_features:
            coverage_analysis["coverage_percentage"] = (
                len(coverage_analysis["covered_by_official"]) / len(custom_features) * 100
            )
        
        return coverage_analysis

def calculate_warning_level(custom_features: List[str], technology_info: Dict[str, Any]) -> str:
    ...
    red_flags = len([f for f in custom_features 
                    if any(flag.lower() in f.lower() 
...
def generate_decision_matrix(technology: str, custom_features: List[str]) -> Dict[str, Any]:
    """Generate decision matrix for integration choices."""
    return {
        "options": [
            {
                "name": "Official Container/SDK",
                "pros": ["Maintained by vendor", "Production ready", "Security updates"],
            ...
            },
            {
                ...
                "cons": ["Development time", "Maintenance burden", "Security responsibility"],
...
def generate_validation_questions(technology: str, custom_features: List[str]) -> List[str]:
    ...
    if "authentication" in " ".join(custom_features).lower():
        base_questions.append("Does the official solution provide authentication features?")
    
    if "api" in " ".join(custom_features).lower():
        base_questions.append("Does the official solution expose the needed API endpoints?")
...
def generate_recommendation(technology: str, custom_features: List[str], technology_info: Dict[str, Any]) -> str:
    ...
    official_solutions = technology_info.get("official_container") or technology_info.get("official_sdks", [])
...
def analyze_integration_text(text: str) -> Dict[str, Any]:
    ...
    custom_indicators = [
        "custom", "build", "implement", "create", "develop", "write",
        "fastapi", "flask", "express", "server", "api", "rest",
        "authentication", "auth", "jwt", "storage", "database"
    ]
    
    detected_custom_work = [indicator for indicator in custom_indicators 
    ...
    for tech in detected_technologies:
        if detected_custom_work:
            ...
            elif tech_warning == "caution" and results["warning_level"] not in ["critical", "warning"]:
