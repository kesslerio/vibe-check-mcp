...
class EnhancedClaudeCliExecutor(ClaudeCliExecutor):
    ...
    def _load_project_context_if_enabled(self) -> Optional[str]:
        ...
        pattern_exceptions_path = (
            self.project_root / ".vibe-check" / "pattern-exceptions.json"
        )
        if pattern_exceptions_path.exists():
            try:
                ...
                if exceptions.get("approved_patterns") or exceptions.get(
                    ...
                    if approved:
                        ...
                        for pattern in approved:
                            reasoning = exceptions.get("reasoning", {}).get(
                                pattern, "No reasoning provided"
                            )
    ...
    def _load_library_context(
        self, detected_libraries: Optional[List[str]] = None
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            libraries_to_include = (
                detected_libraries or list(analysis_context.library_docs.keys())[:5]
            )  # Limit to 5
