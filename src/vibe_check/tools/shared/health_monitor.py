...
class ClaudeCliHealthMonitor:
    ...
    def _update_metrics(self):
        ...
        self.current_metrics.successful_requests_per_minute = len(successful_calls) * 60 / time_span
        
        # Calculate error rates
        total_calls = len(recent_calls)
        if total_calls > 0:
            self.current_metrics.error_rate = (total_calls - len(successful_calls)) / total_calls
    ...
    def _analyze_trends(self):
        ...
        first_success_rate = sum(1 for call in first_half if call.success) / len(first_half)
        second_success_rate = sum(1 for call in second_half if call.success) / len(second_half)
    ...
    def _calculate_health_status(self) -> HealthStatus:
        ...
        if self.circuit_breaker.stats.last_success_time:
            time_since_success = time.time() - self.circuit_breaker.stats.last_success_time
            if time_since_success > self.thresholds.unhealthy_max_time_without_success:
                ...
                issues.append(f"No recent success ({time_since_success/60:.1f} minutes)")
                health_score *= 0.4
        ...
        return {
            ...
            "call_history_summary": {
                ...
                "oldest_record": (
                    ...
                    if self.call_history else None
                ),
                "newest_record": (
    ...
    def _generate_recommendations(self, health_status: HealthStatus) -> List[str]:
        ...
        for issue in health_status.issues:
            if "success rate" in issue.lower():
                recommendations.append("Review error patterns and implement targeted fixes")
                ...
                recommendations.append("Check for persistent issues requiring intervention")
