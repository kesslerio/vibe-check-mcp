...
class QueryIntent:
    ...
    intent_type: (
        str  # tool_evaluation, technical_debt, decision, debugging, implementation
    )
    confidence: float
...
class QueryIntentClassifier:
    ...
    def __init__(self):
        self.vectorizer = TfidfVectorizer(
            max_features=100, ngram_range=(1, 3), stop_words="english", lowercase=True
        )
        ...
        if self.template_vectors is not None and len(self.template_texts) > 0:
            try:
                ...
                similarities = cosine_similarity(
                    query_vector, self.template_vectors
                ).flatten()
    ...
    def _extract_entities(self, text: str) -> List[str]:
        ...
        for i, word in enumerate(words):
            ...
            # Check for code-like terms (contains dash, underscore, or ends with .js, .py, etc.)
                ...
                "-" in word
                or "_" in word
                or any(word.endswith(ext) for ext in [".js", ".py", ".ts", ".go"])
            ):
                entities.append(word.strip(".,!?"))
    ...
    def _extract_context_signals(self, text: str) -> List[str]:
        ...
        if any(
            word in text_lower
            for word in ["failing", "broken", "not working", "errors", "warnings"]
        ):
        ...
        if any(
            word in text_lower
            for word in ["quickly", "asap", "urgent", "deadline", "today", "now"]
        ):
        ...
        if any(
            word in text_lower
            for word in ["worth", "should", "better", "vs", "compare"]
        ):
        ...
        if any(
            word in text_lower
            for word in ["tried", "attempted", "failed", "stuck", "struggling"]
        ):
...
class SemanticResponseMatcher:
    ...
    def __init__(self, response_bank_path: Optional[Path] = None):
        self.response_bank_path = (
            response_bank_path or self._get_default_response_path()
        )
    ...
    def _load_response_bank(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        if not self.response_bank_path.exists():
            logger.warning(
                f"Response bank not found at {self.response_bank_path}, using defaults"
            )
    ...
    def find_best_response(
        self, query: str, intent: QueryIntent, min_similarity: float = 0.2
    ) -> Tuple[str, float]:
        ...
        if self.response_vectors is not None:
            try:
                ...
                enhanced_query = (
                    f"{query} {intent.intent_type} {' '.join(intent.key_entities)}"
                )
                query_vector = self.vectorizer.transform([enhanced_query])
                similarities = cosine_similarity(
                    query_vector, self.response_vectors
                ).flatten()
                ...
                if intent.suggested_response_category:
                    category_indices = [
                        i
                        for i, (cat, _) in enumerate(self.response_mappings)
        ...
        if category_responses:
            ...
            for response_data in category_responses:
                score = sum(
                    1
                    for phrase in response_data.get("key_phrases", [])
    ...
    def _clean_response(self, response: str, intent: QueryIntent) -> str:
        ...
        if intent.key_entities:
            ...
            response = response.replace(
                "{technology}",
                intent.key_entities[0] if intent.key_entities else "the tool",
            )
            response = response.replace(
                "{framework}",
                (
                    intent.key_entities[1]
                    if len(intent.key_entities) > 1
                    else "the framework"
                ),
            )
            response = response.replace(
                "{tech}",
                intent.key_entities[0] if intent.key_entities else "this technology",
            )
