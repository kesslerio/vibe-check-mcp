...
class FileChunker:
    ...
    def _enhance_file_data(self, file_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        changes = enhanced.get("changes", 0) or enhanced.get(
            "additions", 0
        ) + enhanced.get("deletions", 0)
    ...
    def _group_files_into_chunks(
        ...
        for file_data in sorted_files:
            ...
            if file_changes > self.max_lines_per_chunk:
                ...
                if current_chunk_files:
                    chunks.append(
                        self._create_chunk(
                            chunk_id, current_chunk_files, current_chunk_lines
                        )
                    )
                    chunk_id += 1
                    current_chunk_files = []
            ...
            if (
                current_chunk_lines + file_changes > self.max_lines_per_chunk
        ...
        if current_chunk_files:
            chunks.append(
                self._create_chunk(chunk_id, current_chunk_files, current_chunk_lines)
            )

        return chunks

    def _create_chunk(
        self, chunk_id: int, files: List[Dict[str, Any]], total_lines: int
    ) -> FileChunk:
...
class ChunkedAnalyzer:
    ...
    async def _analyze_chunks_concurrently(
        self, chunks: List[FileChunk], pr_data: Dict[str, Any]
        ...
        tasks = [
            self._analyze_single_chunk(chunk, pr_data, semaphore) for chunk in chunks
        ]
    ...
    async def _analyze_single_chunk(
        self, chunk: FileChunk, pr_data: Dict[str, Any], semaphore: asyncio.Semaphore
    ) -> ChunkAnalysisResult:
    ...
    def _build_chunk_analysis_prompt(
        self, chunk: FileChunk, pr_data: Dict[str, Any]
    ) -> str:
...
"""
    ...
    def _parse_chunk_analysis(
        self, chunk: FileChunk, analysis_output: str, duration: float
    ) -> ChunkAnalysisResult:
    ...
    def _categorize_pattern(self, pattern: str) -> str:
        ...
        if any(
            word in pattern_lower for word in ["security", "vulnerability", "unsafe"]
        ):
            return "security"
        elif any(
            word in pattern_lower for word in ["performance", "optimization", "slow"]
        ):
            return "performance"
        elif any(
            word in pattern_lower for word in ["bug", "error", "issue", "problem"]
        ):
            return "bug_risk"
            ...
            word in pattern_lower for word in ["structure", "organization", "design"]
        ):
            return "architecture"
        else:
            return "general"
    ...
    def _create_overall_assessment(
        ...
        if success_rate >= 1.0:
            ...
            assessment_parts.append(
                f"❌ Limited analysis due to failures ({len(successful_results)}/{total_chunks} chunks)"
            )
        ...
        if failed_results:
            ...
            assessment_parts.append(
                f"Failed to analyze: {failed_files} files due to errors"
            )
...
async def analyze_pr_with_chunking(
    pr_data: Dict[str, Any], pr_files: List[Dict[str, Any]]
) -> ChunkedAnalysisResult:
    """
