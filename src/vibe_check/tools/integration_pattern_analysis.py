...
def analyze_integration_patterns_fast(
    ...
    """
    logger.info(f"Fast integration pattern analysis requested for {len(content)} characters")
    ...
    try:
        ...
        if detail_level in ["standard", "comprehensive"]:
            result["educational_content"] = _get_educational_content(analysis, detail_level)
...
def quick_technology_scan(content: str) -> Dict[str, Any]:
    ...
    try:
        ...
        analysis = detector.analyze_integration_patterns(content, include_line_analysis=False)
        tech_details = []
        ...
        for tech in analysis.detected_technologies:
            ...
            if tech.red_flags:
                ...
                tech_info["warning"] = f"⚠️ Avoid custom {tech.red_flags[0]} - official alternative available"
        ...
        return {
            ...
            "quick_recommendations": [
                f"Research {tech.technology} official solution" for tech in analysis.detected_technologies
...
def analyze_effort_complexity(
    content: str,
    pr_metrics: Optional[Dict[str, int]] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if pr_metrics:
            ...
            if total_changes > 1000 and analysis.detected_technologies:
                ...
                effort_summary["warning"] = f"🚨 {total_changes} lines for integration - verify necessity"
            elif total_changes > 500 and analysis.detected_technologies:
                effort_summary["complexity_assessment"] = "medium"
                effort_summary["caution"] = f"⚠️ {total_changes} lines for integration - check official alternatives"
            else:
...
def _get_case_studies(analysis) -> Dict[str, Any]:
    ...
    if any(p.pattern_type == "integration_over_engineering" for p in analysis.integration_anti_patterns):
...
def enhance_text_analysis_with_integration_patterns(
    ...
    try:
        ...
        enhanced_analysis["integration_insights"] = {
            "technologies_detected": [t.technology for t in integration_analysis.detected_technologies],
            "integration_warning_level": integration_analysis.warning_level,
            "integration_recommendations": integration_analysis.recommendations[:3],  # Top 3
            "has_integration_anti_patterns": len(integration_analysis.integration_anti_patterns) > 0
        }
