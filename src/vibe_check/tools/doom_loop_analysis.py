...
def get_doom_loop_context_for_tool(tool_name: str) -> Optional[Dict[str, Any]]:
    ...
    if loop_pattern.severity in [LoopSeverity.WARNING, LoopSeverity.CRITICAL, LoopSeverity.EMERGENCY]:
        return {
            "doom_loop_detected": True,
            "severity": loop_pattern.severity.value,
            "warning_message": _format_contextual_warning(loop_pattern),
            "quick_suggestions": loop_pattern.intervention_suggestions[:2],  # Top 2 suggestions
...
def analyze_text_for_doom_loops(
    ...
    if text_pattern and session_pattern:
        primary_pattern = text_pattern if detector._severity_score(text_pattern.severity) > detector._severity_score(session_pattern.severity) else session_pattern
    elif text_pattern:
...
def get_session_health_analysis() -> Dict[str, Any]:
    ...
    if health_report["doom_loop_detected"]:
        ...
        health_report["action_required"] = health_report["severity"] in ["critical", "emergency"]
    else:
        health_report["user_message"] = f"✅ Session health: Good ({health_report['health_score']}/100)"
...
def force_doom_loop_intervention() -> Dict[str, Any]:
    ...
    emergency_pattern = LoopPattern(
        ...
        duration_minutes=detector.current_session.session_duration_minutes if detector.current_session else 0,
...
def reset_session_tracking() -> Dict[str, Any]:
    ...
    old_session_info = {
        "duration": detector.current_session.session_duration_minutes if detector.current_session else 0,
        "calls": len(detector.current_session.mcp_calls) if detector.current_session else 0
    }
...
def _format_health_message(health_report: Dict[str, Any]) -> str:
    ...
    if severity == "emergency":
        ...
        return f"⚠️ WARNING: Analysis patterns detected. Health score: {health_score}/100"
    else:
