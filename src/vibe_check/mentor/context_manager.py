...
class ContextCache:
    ...
    def find_project_containing_files(self, file_names: List[str], search_dirs: List[str] = None) -> Optional[str]:
                ...
                for root, dirs, files in os.walk(base_dir):
                    ...
                    for target_file in file_names:
                        if os.path.basename(target_file) in files:
                            ...
                            full_path = os.path.join(root, os.path.basename(target_file))
    ...
    def load_project_registry(self) -> Dict[str, str]:
        ...
        if PROJECT_REGISTRY_PATH.exists():
            try:
                ...
                logger.warning(f"Failed to load project registry from {PROJECT_REGISTRY_PATH}")
        return {}
    ...
    def auto_discover_context(
        ...
        if not working_directory:
            ...
            if not working_directory and mentioned_files:
                ...
                if working_directory:
                    logger.info(f"Auto-discovered project at {working_directory} containing {mentioned_files}")
            ...
            if not working_directory:
                ...
                for project_name, project_path in registry.items():
                    if project_name.lower() in query.lower():
                        ...
                        logger.info(f"Found registered project '{project_name}' at {project_path}")
                        break
