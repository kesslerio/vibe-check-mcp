...
DANGEROUS_STRINGS = frozenset(['<script', 'javascript:', 'eval(', 'exec(', '__import__', 'drop table', 'delete from', '; --'])
SECRET_INDICATORS = frozenset(['api', 'key', 'secret', 'token', 'password', 'bearer', 'ghp_'])
RESTRICTED_PATHS = frozenset(['/etc/', '/proc/', '/sys/', '/dev/', '.ssh/', '.aws/'])
ALLOWED_EXTENSIONS = frozenset(['.py', '.js', '.ts', '.jsx', '.tsx', '.java', '.go', '.rs', '.json', '.yaml', '.yml', '.md', '.txt'])


def validate_query_ultrafast(query: str, intent: Optional[str] = None) -> Tuple[bool, Dict[str, Any]]:
    """
    ...
    try:
        ...
        if original_generate:
            async def ultrafast_generate_dynamic_response(
                ...
                # Ultra-fast workspace validation
                if workspace_data:
                    is_valid, sanitized_ws = validate_workspace_ultrafast(workspace_data)
                    if is_valid:
                        workspace_data = sanitized_ws
                        # Ultra-fast secrets scan
                        if "code" in workspace_data:
                            workspace_data["code"] = scan_secrets_ultrafast(workspace_data["code"])
            ...
            original.MCPSamplingClient.generate_dynamic_response = ultrafast_generate_dynamic_response
...
# Minimal compatibility classes for existing imports
...
class PromptBuilder:
    ...
    def build_prompt(self, intent: str, query: str, context: Dict[str, Any]) -> str:
        return f"Intent: {intent}\nQuery: {query}\nContext: {str(context)}"

class ResponseCache:
    """Minimal response cache"""
    def __init__(self):
        self.cache = {}
...
class MCPSamplingClient:
    """Minimal MCP sampling client with security patches"""
    ...
    async def generate_dynamic_response(
        ...
        """Generate response with ultra-fast security checks"""
        
        # Ultra-fast rate limit
        if not await check_rate_limit_ultrafast(user_id):
            return {"content": "Rate limited", "generated": False}
        ...
        # Return simple response (no actual MCP call in this minimal version)
        return {
            "content": f"Response for {intent}: {sanitized['query'][:100]}",
            "generated": True,
            "quality": ResponseQuality.HIGH.value
        }
