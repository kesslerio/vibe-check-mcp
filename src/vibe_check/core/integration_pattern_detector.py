...
class IntegrationPatternDetector:
    ...
    def analyze_integration_patterns(
        ...
        core_patterns = self.base_detector.analyze_text_for_patterns(
            content=content,
            context=context,
            focus_patterns=["infrastructure_without_implementation", "integration_over_engineering"]
        )
        
        # Step 3: Run integration-specific detection
        integration_patterns = self._detect_integration_patterns(full_text, detected_technologies)
        ...
        recommendations = self._generate_recommendations(
            ...
            core_patterns + integration_patterns,
            effort_analysis
        )
    ...
    def _detect_technologies(self, text: str) -> List[TechnologyDetection]:
        ...
        for tech_name, config in self.technology_patterns.items():
            ...
            if confidence >= 0.3:  # Threshold for detection
                detected.append(TechnologyDetection(
    ...
    def _detect_integration_patterns(
        ...
        if "integration_over_engineering" in self.base_detector.patterns:
            ...
            if result.detected and detected_technologies:
                ...
                result.evidence.append(f"Detected technologies: {', '.join(tech_names)}")
                ...
                if tech.red_flags:
                    ...
                    if red_flags_found:
                        ...
                        tech_pattern = DetectionResult(
                            ...
                            evidence=[f"{tech.technology} red flag detected: {flag}" for flag in red_flags_found],
                            threshold=0.5,
    ...
    def _calculate_warning_level(
        ...
        if len(high_confidence_patterns) >= 2 or red_flag_count >= 3:
            ...
            return "warning"
        elif len(medium_confidence_patterns) >= 1 or red_flag_count >= 1 or tech_with_solutions >= 2:
            return "caution"
    ...
    def _generate_research_questions(self, technologies: List[TechnologyDetection]) -> List[str]:
