...
class DoomLoopDetector:
    ...
    def analyze_current_session(self) -> Optional[LoopPattern]:
        ...
        if detected_patterns:
            most_severe = max(detected_patterns, key=lambda p: self._severity_score(p.severity))
            return most_severe
        
        return None
    
    def get_intervention_recommendation(self, loop_pattern: LoopPattern) -> Dict[str, Any]:
        """
    ...
    def check_text_for_loop_patterns(self, text: str, context: str = "") -> Optional[LoopPattern]:
        """
        ...
        paralysis_count = sum(1 for pattern in paralysis_indicators 
        ...
        cycling_count = sum(1 for pattern in cycling_indicators 
                          if re.search(pattern, full_text))
        ...
        overthinking_count = sum(1 for pattern in overthinking_indicators 
                               if re.search(pattern, full_text))
        ...
        if paralysis_count >= 2:
            evidence.append(f"Analysis paralysis language detected ({paralysis_count} indicators)")
        if cycling_count >= 1:
            evidence.append(f"Decision cycling patterns detected ({cycling_count} indicators)")
        if overthinking_count >= 2:
            evidence.append(f"Overthinking patterns detected ({overthinking_count} indicators)")
    ...
    def get_session_health_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "unique_tools_used": len(set(call["tool_name"] for call in self.current_session.mcp_calls)),
            ...
            "recommendations": self.get_intervention_recommendation(loop_pattern) if loop_pattern else None
        }
    ...
    def _check_temporal_patterns(self) -> Optional[LoopPattern]:
        ...
        if duration >= 60:
            return LoopPattern(
                ...
                evidence=[f"Session running for {duration} minutes without clear progress"],
            ...
            return LoopPattern(
                ...
                severity=LoopSeverity.CRITICAL,
                duration_minutes=duration,
                evidence=[f"Extended session ({duration} minutes) suggests analysis paralysis"],
            ...
            return LoopPattern(
                ...
                severity=LoopSeverity.WARNING,
                duration_minutes=duration,
                evidence=[f"Session length ({duration} minutes) approaching analysis paralysis"],
    ...
    def _check_repetition_patterns(self) -> Optional[LoopPattern]:
        ...
        repeated_tools = {tool: count for tool, count in self.current_session.repeated_calls.items() 
                         if count >= 3}
        ...
        return LoopPattern(
            ...
            severity=severity,
            duration_minutes=self.current_session.session_duration_minutes,
            evidence=[f"Tool '{most_repeated_tool}' called {max_repetitions} times", 
    ...
    def _check_decision_paralysis(self) -> Optional[LoopPattern]:
        ...
        if len(self.current_session.decision_patterns) >= 3:
            return LoopPattern(
                ...
                severity=LoopSeverity.WARNING,
                duration_minutes=self.current_session.session_duration_minutes,
                evidence=[f"Multiple decision-making discussions ({len(self.current_session.decision_patterns)})",
    ...
    def _check_topic_cycling(self) -> Optional[LoopPattern]:
        ...
        if len(self.current_session.topics_discussed) <= 2 and len(self.current_session.mcp_calls) >= 5:
            return LoopPattern(
                pattern_type="topic_cycling",
                severity=LoopSeverity.WARNING,
                duration_minutes=self.current_session.session_duration_minutes,
                evidence=[f"Only {len(self.current_session.topics_discussed)} topics in {len(self.current_session.mcp_calls)} calls",
    ...
    def _calculate_health_score(self) -> int:
        ...
        max_repetitions = max(self.current_session.repeated_calls.values()) if self.current_session.repeated_calls else 0
        if max_repetitions > 5:
            score -= 30
        ...
        if (len(self.current_session.topics_discussed) <= 2 and 
            len(self.current_session.mcp_calls) >= 5):
            score -= 15
    ...
    def _generate_decision_forcing_questions(self, loop_pattern: LoopPattern) -> List[str]:
