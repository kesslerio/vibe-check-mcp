...
class BusinessContext:
    ...
    def is_planning_phase(self) -> bool:
        return self.primary_type in [
            ...
            ContextType.IMPLEMENTATION_PROPOSAL,
        ]

    @property
    def needs_clarification(self) -> bool:
        return (
            self.confidence < ConfidenceLevel.MEDIUM.value
            or len(self.questions_needed) > 0
        )
...
class BusinessContextExtractor:
    ...
    COMPLETION_PATTERNS = [
        (
            r"\b(?:completed|finished|implemented|done|shipped)\s+(?:the\s+)?(?:issue|feature|story|ticket|task|pr|pull\s+request)\s*#?\d+",
            0.9,
        ),
        (
            r"\b(?:just|already|successfully)?\s*(?:completed|finished|implemented|shipped|deployed|released)\s+(?:the\s+)?(?:implementation|feature|change|update|integration)",
            0.8,
        ),
        (
            r"\b(?:here\'s|this\s+is)\s+(?:what\s+i|the)\s+(?:did|implemented|built|created)",
            0.7,
        ),
        (
            r"\b(?:i\'ve|we\'ve)\s+(?:just|already)?\s*(?:implemented|built|created|updated|changed)",
            0.7,
        ),
        (
    ...
    REVIEW_PATTERNS = [
        (
            r"\b(?:did\s+i\s+miss|what\s+did\s+i\s+miss|anything\s+(?:i\s+)?(?:missed|forgot))",
            0.9,
        ),
        (
            r"\b(?:feedback\s+on|review\s+(?:of|this)|thoughts\s+on|look\s+good|check\s+(?:my|this))",
            0.8,
        ),
        (r"\b(?:any\s+)?(?:suggestions|improvements|concerns|issues)\s*\?", 0.6),
        (
            r"\b(?:what\s+do\s+you\s+think|how\s+does\s+this\s+look|is\s+this\s+correct)",
            0.7,
        ),
    ]
    ...
    PLANNING_PATTERNS = [
        (
            r"\b(?:planning|going|want|need)\s+to\s+(?:build|implement|create|develop)",
            0.8,
        ),
        (
            r"\b(?:should\s+i|should\s+we|can\s+i|can\s+we)\s+(?:build|implement|create|use)",
            0.7,
        ),
        (r"\b(?:thinking\s+(?:of|about)|considering|evaluating|exploring)", 0.6),
        (
            r"\b(?:how\s+to|best\s+way\s+to|approach\s+for)\s+(?:implement|build|create)",
            0.7,
        ),
    ...
    PROCESS_PATTERNS = [
        (
            r"\b(?:strategy|approach|method|process)\s+(?:update|change|migration|refactor)",
            0.7,
        ),
        (r"\b(?:changed|updated|modified)\s+from\s+.+\s+to\s+", 0.8),
        (
            r"\b(?:three|3)-(?:path|step|phase|tier)\s+(?:approach|strategy|process)",
            0.7,
        ),
        (
            r"\b(?:workflow|pipeline|architecture)\s+(?:design|implementation|update)",
            0.6,
        ),
    ]
    ...
    def extract_context(
        ...
        if phase:
            ...
            if phase_lower == "planning":
                ...
                logger.info(
                    f"Explicit phase=planning detected, bypassing ambiguity detection"
                )
                ...
                questions = []
                detected_patterns = [
                    f"explicit_phase: {phase_lower} (confidence: {confidence})"
                ]
                logger.info(
                    f"Explicit phase={phase_lower} detected, bypassing ambiguity detection"
                )
                ...
                questions = []
                detected_patterns = [
                    f"explicit_phase: review (confidence: {confidence})"
                ]
                logger.info(
                    f"Explicit phase=review detected, bypassing ambiguity detection"
                )
            else:
                # Unknown phase - fall back to pattern detection
                logger.warning(
                    f"Unknown explicit phase '{phase}', falling back to pattern detection"
                )
        ...
        if not phase:
            ...
            completion_score = self._check_patterns(
                full_text, self.COMPLETION_PATTERNS, "completion"
            )
            detected_patterns.extend(completion_score[2])

            # Check for review requests
            review_score = self._check_patterns(
                full_text, self.REVIEW_PATTERNS, "review"
            )
            detected_patterns.extend(review_score[2])

            # Check for planning indicators
            planning_score = self._check_patterns(
                full_text, self.PLANNING_PATTERNS, "planning"
            )
            detected_patterns.extend(planning_score[2])

            # Check for process descriptions
            process_score = self._check_patterns(
                full_text, self.PROCESS_PATTERNS, "process"
            )
    ...
    def _check_patterns(
        self, text: str, patterns: List[Tuple[str, float]], pattern_type: str
        ...
        for pattern, weight in patterns:
            try:
                ...
                logger.warning(
                    f"Regex pattern error in {pattern_type}: {pattern[:50]}... - {str(e)}"
                )
                continue
            except Exception as e:
                logger.error(
                    f"Unexpected error checking pattern in {pattern_type}: {str(e)}"
                )
                continue
    ...
    def _determine_context_type(
        ...
        if max_score < ConfidenceThresholds.LOW:
            return (
                ContextType.UNKNOWN,
                ConfidenceLevel.UNCERTAIN.value,
        ...
        if (
            completion_score > ConfidenceThresholds.MEDIUM
            and review_score > ConfidenceThresholds.MEDIUM
        ):
            ...
            completion_score > ConfidenceThresholds.LOW
            and planning_score > ConfidenceThresholds.LOW
        ):
            # Mixed completion and planning signals - lower confidence
            primary_type = (
                ContextType.COMPLETION_REPORT
                if completion_score > planning_score
                else ContextType.PLANNING_DISCUSSION
            )
            confidence = ConfidenceLevel.MEDIUM.value
            questions = [
                "Are you describing completed work or planning future implementation?"
            ]
        else:
            ...
            if (
                score_diff > ConfidenceThresholds.MEDIUM
                and max_score > ConfidenceThresholds.HIGH
            ):
                confidence = ConfidenceLevel.HIGH.value
                questions = []
            elif (
                score_diff > ConfidenceThresholds.LOW
                or max_score > ConfidenceThresholds.MEDIUM
            ):
                confidence = ConfidenceLevel.MEDIUM.value
                questions = (
                    self._generate_clarifying_questions(full_text, primary_type.value)
                    if max_score < ConfidenceThresholds.HIGH
                    else []
                )
            else:
                confidence = ConfidenceLevel.LOW.value
                questions = self._generate_clarifying_questions(
                    full_text, primary_type.value
                )
    ...
    def _generate_clarifying_questions(self, text: str, context_type: str) -> List[str]:
        ...
        if re.search(r"\b(?:implement|build|create)", text, re.IGNORECASE):
            has_completion_words = re.search(
                r"\b(?:completed|finished|done|implemented|shipped)",
                text,
                re.IGNORECASE,
            )
            has_planning_words = re.search(
                r"\b(?:planning|going|want)\s+to", text, re.IGNORECASE
            )
        ...
        if context_type == "review_request":
            questions.append(
                "Are you looking for feedback on completed work, or guidance on how to approach this?"
            )
    ...
    def _extract_metadata(self, text: str) -> Dict[str, any]:
        ...
        issue_match = re.search(
            r"(?:issue|pr|pull\s+request)\s*#?(\d+)", text, re.IGNORECASE
        )
        if issue_match:
        ...
        action_verbs = re.findall(
            r"\b(?:implemented|built|created|updated|changed|planning|considering|completed)\b",
            text,
            re.IGNORECASE,
        )
        if action_verbs:
