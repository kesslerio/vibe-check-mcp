...
class PatternDetector:
    ...
    def detect_all_patterns(self, content: str, context: str = "") -> List[Dict[str, Any]]:
...
def validate_with_cognee_case() -> Dict[str, Any]:
    ...
    validation_passed = (
        result["detected"] == expected["should_detect"] and
        result["confidence"] >= expected["minimum_confidence"]
    )
...
def validate_with_good_case() -> Dict[str, Any]:
    ...
    validation_passed = (
        result["detected"] == expected_detection and
        result["confidence"] < 0.3  # Low confidence for good cases
    )
...
def validate_additional_cases() -> List[Dict[str, Any]]:
    ...
    for case in test_cases:
        ...
        if case["should_detect"]:
            ...
            validation_passed = (
                not result["detected"] and
                result["confidence"] <= case.get("max_confidence", 0.4)
            )
...
def run_comprehensive_validation() -> Dict[str, Any]:
    ...
    print(f"Confidence: {cognee_result['actual_confidence']:.2f} (min: {cognee_result['expected_min_confidence']:.2f})")
    ...
    for i, result in enumerate(additional_results, 1):
        ...
        print(f"   Confidence: {result['confidence']:.2f}, Detected: {result['actual_detection']}")
